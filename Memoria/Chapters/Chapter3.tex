...
\begin{figure}[H]
	\centering
	\includegraphics[width=.5\textwidth]{./Figures/QL76_real.jpeg}
...
Para finalizar, se inicializan las colas, las cuales son los canales de comunicación entre las distintas tareas del programa y se crea la tarea \textit{init\_mqtt\_server\_task()}, que se explica a continuación. 

En el mismo diagrama de la figura \ref{fig:flujo_firmware_1} (bifurcación A), se muestra el flujo de la tarea \textit{init\_mqtt\_server\_task()}. Esta empieza por inicializar los serivicios MQTT en el módulo SIM A7670SA, para luego iniciar la conexión con el \textbf{Broker} MQTT en la nube. Si lo anterior ocurre sin problemas, esta tarea procede a crear las demás tareas del sistema, a saber: \textit{gnss\_task()}, \textit{collect\_data\_task()}, \textit{transmit\_server\_task()} y \textit{lcd\_task()}. Luego, el \textit{scheduler} de \textit{FreeRtos}, desencadena la multitarea. Este punto se representa con la bifurcación B. Para finalizar, la tarea \textit{init\_mqtt\_server\_task()} es eliminada. 
...
\item Tarea \textit{gnss\_task()}, procesa datos GNSS capturados por UART\_0, envía las posiciones procesadas a \textit{collect\_data\_task()} a través de la cola \textit{position\_queue} y verifica si la posición ha cambiado en un intervalo de 5 minutos para entrar en modo de ahorro de energía si no hay cambios.

\item Tarea \textit{collect\_data\_task()}, monitorea eventos de ocupación y posición, guarda los datos de ocupación y posición cuando ocurren eventos y envía los datos de ocupación a \textit{transmit\_server\_task()} a través de la cola \textit{cava\_data\_queue} para su transmisión al servidor.
...
\item Tarea \textit{transmit\_to\_server\_task()} verifica continuamente si hay eventos de datos de la cava, transmite los datos de la cava al servidor MQTT cuando hay un evento y envía los datos a la cola \textit{lcd\_queue} para ser procesados por \textit{lcd\_task()}.

\item Tarea \textit{lcd\_task()} verifica continuamente si hay eventos en la cola \textit{lcd\_queue} y muestra la información de posición, ocupación y estado del servidor en la pantalla LCD cuando hay un evento.
...
En el código \ref{cod:gnss_task}, se presenta el pseudocódigo de la tarea \textit{gnss\_task()}. La tarea se encarga de procesar las tramas NMEA que son datos provenientes del receptor GNSS, envía por la cola \textit{position\_queue}, que es gestionada por la tarea \textit{collect\_data\_task()}. El sistema verifica si ha pasado un intervalo de tiempo de 5 minutos, para comprar: 
...
\begin{figure}[H]
\begin{lstlisting}[label=cod:gnss_task, caption=Pseudocódigo de la tarea gnss\_task().] 
...
En el código \ref{cod:occupancy_isr_handler}, se presenta el pseudocódigo del manejador de interrupciones \textit{occupancy\_isr\_handler()}, que aunque no es una tarea mapeada en el \textit{scheduler} de \textit{FreeRtos}, cumple un papel primordial, puesto que permite capturar los eventos de los botones de ocupación del sistema. El manejador de interrupciones se activa cuando se presiona un botón, se captura cuál botón ha sido presionado. Si el botón presionado es el de "libre" se actualiza el estado para indicar que el recurso está libre y se envía un estado "Libre" a través de \textit{occupancy\_queue}. Si el botón presionado es el de "ocupado", se actualiza el estado para indicar que la cava está ocupada y se envía un estado "Ocupado" a través de \textit{occupancy\_queue}.
...
En el código \ref{cod:collect_data_task}, se presenta el pseudocódigo de la tarea \textit{collect\_data\_task()}. La tarea verifica si hay un evento de ocupación, si este ocurre, se guarda la información correspondiente. También verifica si hay un evento de posición, si ocurre, se guarda la información correspondiente. Finalmente, la información de ocupación se envía por la cola \textit{cava\_data\_queue} que es gestionada por la tarea \textit{transmit\_to\_server\_task()}.


 
\begin{figure}[H]
\begin{lstlisting}[label=cod:collect_data_task, caption=Pseudocódigo de la tarea collect\_data\_task().] 
...
En el código \ref{cod:lcd_task}, se presenta el pseudocódigo de la tarea \textit{lcd\_task()}. Esta tarea verifica si hay un evento en la cola, si no hay evento, el sistema vuelve al inicio del bucle; si hay un evento, se muestra la siguiente información en la pantalla LCD: 
	\begin{itemize}
...
\begin{figure}[H]
\begin{lstlisting}[label=cod:lcd_task, caption=Pseudocódigo de la tarea lcd\_task().] 
...
Un driver para el manejo de la pantalla \textit{LCD RGB Backlight} de 16x2 que se llamó \textit{lcd\_i2c\_grove.h}. Esta pantalla presenta interfaz de comunicación I2C, para reducir la cantidad de pines utilizados por el microcontrolador. Este driver presenta las funciones descritas en el código \ref{lst:driver_lcd}. Este driver fue adaptado del driver proporcionado por el fabricante en: \url{https://github.com/Seeed-Studio/Grove_LCD_RGB_Backlight}.
...
\begin{enumerate}
	...
	\item \textbf{Gestión de Mensajes:}
		\begin{itemize}
		\item Se define la función \textit{get\_cava\_data()} para procesar los mensajes recibidos del \textit{Broker}. Se convierten los mensajes a formato JSON y se extraen los datos de la cava. Se actualiza el mapa con la nueva posición de la cava, los demás datos actualizados y se muestra la ruta seguida.
		\end{itemize}
...
La primera tarjeta (<div class="card mb-3">) tiene un cuerpo (<div class="card-body">) que muestra el título <<Dispositivo:>> y un párrafo con el ID \textit{cava\_name} para mostrar el nombre del dispositivo.
...
El programa declara un mapa (\textit{map}), un marcador (\textit{marker}), una lista para almacenar el rastro de posiciones (\textit{path}), y una variable para la línea que representará ese rastro (\textit{polyline}). Además, define una posición inicial (\textit{home}) con coordenadas específicas y una variable para la posición actual de la cava (\textit{current\_cava\_position}).

A continuación, se configura una serie de opciones para conectarse a un servidor MQTT, incluyendo el tiempo de espera para la conexión, el ID del cliente, el nombre de usuario, la contraseña y el intervalo de mantenimiento. También se especifica la URL para la conexión (\textit{connectUrl}) y crea un cliente MQTT con estas opciones. Define un tema (\textit{topic\_cava}) al que la web se suscribe para recibir datos de la cava y declara una variable (\textit{cava\_data}) para almacenar estos datos.

Se definen las siguientes funciones: 

\begin{itemize}

\item La función \textit{init\_map} inicializa el mapa utilizando OpenStreetMap. Centra el mapa en la posición inicial (\textit{home}) y añade un marcador en esta posición. Además, crea un \textit{popup} para el marcador que muestra la latitud y longitud de la posición inicial.

\item La función \textit{get\_cava\_data} recibe datos de la cava, convierte las coordenadas de latitud y longitud de estos datos a números flotantes, y devuelve un objeto con estas coordenadas.

\item La función \textit{set\_cava\_position\_map} se encarga de actualizar el mapa con la nueva posición de la cava. Primero, actualiza elementos HTML con los nuevos valores de latitud y longitud. Si ya existe un marcador, lo elimina del mapa. Luego, crea un nuevo marcador en la nueva posición y añade un \textit{popup} con la información de esta posición. Además, añade la nueva posición a la lista path y dibuja un rastro en el mapa con esta lista. Finalmente, ajusta la vista del mapa para centrarlo en la nueva posición.

\end{itemize}

El programa llama a \textit{init\_map} para inicializar el mapa. Cuando el cliente MQTT se conecta, imprime un mensaje de 'Conectado' y se suscribe al tema \textit{topic\_cava}. Maneja eventos de reconexión e impresión de errores en caso de que falle la conexión. Cuando el cliente recibe un mensaje, convierte el mensaje a un objeto JSON, obtiene la posición de la cava utilizando \textit{get\_cava\_data}, y llama a \textit{set\_cava\_position\_map} con esta posición para actualizar el mapa.
