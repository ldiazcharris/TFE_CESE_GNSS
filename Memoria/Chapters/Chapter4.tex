...
Este capítulo presenta los ensayos realizados para evaluar el desempeño del prototipo y los resultados obtenidos. Los ensayos se desarrollaron para validar la funcionalidad y exactitud del sistema en condiciones operativas de laboratorio. 
...
\item En la PCB desarrollada:
	...
	\item Se midió el consumo de corriente de todo el hardware a plena carga, es decir, en el momento en que todas las tareas para las que fue diseñado están en ejecución. 
	\end{enumerate}
...
La medición de la corriente total consumida por el hardware se llevó a cabo por un período de 30 segundos contados a partir de que se enciende el dispositivo; este es el tiempo que le toma al sistema inicializarse y comenzar a enviar datos al servidor.  Resultó que existen variaciones desde los 130 mA hasta los 190 mA, además de valores con picos de 210 mA, de corta duración. 
...
\begin{enumerate}
\item Para esta prueba se requiere el uso del puerto UART\_1, por lo tanto, se intercambian los pines GND, Vin, Tx y Rx del módulo Quectel L76 en la ranura del módulo SIM A7670SA. Debido a que son conectores diferentes se deben emplear cables de prueba.

\item Se deben desconectar los módulos SIM A7670SA y Quectel L76 de sus respectivas posiciones designadas, para no interferir la comunicación serial. 
\end{enumerate}
...
\end{lstlisting}
\captionof{lstlisting}{Código para probar la comunicación con el módulo Quectel L76.}
\label{cod:cod_proof_gnss}
\end{figure}


Luego de implementar las configuraciones anteriores, se obtuvieron las tramas NMEA proporcionadas por el módulo Quectel L76. La prueba resultó exitosa, ya que es posible capturar dichas tramas y procesarlas. En la figura \ref{fig:pru_QL76_recep} se muestra una captura de las tramas visualizadas en la terminal \textit{Hercules}. En esta captura se puede observar las tramas que incian con \$GNRMC, las cuales contienen la información de geolocalización.  
...
En la figura \ref{fig:pru_A7670_recep} se muestra una captura de los mensajes visualizados en la terminal \textit{Hercules}. En esta captura se puede observar mensajes recibidos del módulo SIM A7670SA que inician los símbolos + y *, que contienen información sobre los procesos del módulo.   
...
En la figura \ref{fig:pru_A7670_transm} se muestra otra captura de mensajes visualizados en la terminal \textit{Hercules}. En esta captura se muestra la transmisión del comando \verb|AT+CMGD=1,4| hacia el módulo SIM A7670SA, este comando debe cumplir con las siguientes condiciones: 

\begin{enumerate}
\item Debe iniciar con los caracteres \verb|AT+|.
\item Comando a enviar. En el caso de la prueba realizada, se envió el comando CMGD que se usa para eliminar mensajes SMS. 
\item Los dos parámetros del comando: en este caso \verb|<index>(1)| y \verb|<delflag>(4)|, se utilizan para borrar todos los mensajes en la tarjeta SIM.
\item Por último, los símbolos de retorno de carro \verb|<CR>| y salto de línea \verb|<LF>|. Si estos dos símbolos no están presentes, el comando no será reconocido.
\end{enumerate}

En la figura \ref{fig:pru_A7670_transm} también se observa la respuesta del módulo al comando enviado. El módulo responde con un texto que contiene el comando y ``OK'' para indicar que fue ejecutado con éxito. 
...
Se implementó un código de prueba (ver código \ref{cod:cod_proof_lcd}) que permitió usar las funciones proporcionadas por el driver \verb|lcd_i2c_grove.h|. Este programa escribe textos y cambia los colores de la pantalla LCD cada tres segundos. Además, borra la pantalla cada vez que escribe un texto nuevo. De esta manera se puede corroborar el funcionamiento de la LCD. En la figura \ref{fig:prueba_lcd} se muestran fotografías de los resultados de la prueba. 
...
\begin{enumerate}
\item Se seleccionaron posiciones en puntos fijos en la ciudad de Barranquilla. En un conjunto de edificios residenciales con calles internas para recrear espacios urbanos con presencia de edificios. 
...
\item Se calculó la distancia entre las coordenadas registradas por el prototipo y las coordenadas tomadas de referencia de \textit{Google Map}. El cálculo de esta distancia se realizó con la ecuación de \textit{Haversine} \citep{Alkan-Celebi}. El cálculo se toma como el grado de exactitud en la geolocalización realizada por el prototipo, al considerar como valor verdadero el proporcionado por el teléfono inteligente.
...
Luego de realizadas las pruebas se tabularon los datos obtenidos (ver tabla \ref{tabla:distancias_calculadas}). 

Las muestras 1 a 8 de la tabla \ref{tabla:distancias_calculadas}, corresponden a posiciones en las calles, con edificios al rededor. En estos registros se observa que la exactitud está entre 0 y 5 m.

Por otro lado, las muestras 9 a 11 de la tabla \ref{tabla:distancias_calculadas} corresponden a coordenadas medidas dentro de los edificios, en los pasillos. Como era de esperar, el hormigón armado ofrece interferencia a la señal satelital, por lo tanto la exactitud de la medición baja considerablemente a alrededor de 40 m.
